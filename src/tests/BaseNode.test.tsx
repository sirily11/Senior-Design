...
describe("Test number of leaves", () => {
    let solutionNode: NodeObj = { id: "1", title: "solution", description: "Solution Node", nodeType: NodeTypes.solution, connection: []}
    let goalNode: NodeObj = { id: "0", title: "Root", description: "Root Node", nodeType: NodeTypes.goal, connection: [solutionNode] }

    test("Get number of leaves", () => {
        let graph = new BaseGraphObject({ _id: "test_graph", name: "Test", description: "Test graph", nodes: [goalNode, solutionNode] })
        let number = graph.nodes[0]._getNumberOfLeaves(graph.nodes[0])
